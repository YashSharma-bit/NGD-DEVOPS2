...
def load_geo_with_index() -> gpd.GeoDataFrame:
    ...
    if not geo_path.exists():
        # Fall back to tabular data (no geometry — charts only, no maps)
        logger.warning(
            "districts_geo.parquet not found — falling back to tabular data. "
            "Choropleth maps will be skipped, charts will still be generated."
        )
        for fallback in ["districts_merged", "census_primary_raw"]:
            p = PROCESSED / f"{fallback}.parquet"
            if p.exists():
        ...
        raise FileNotFoundError(
            "No district data found in data/processed/. Run clean_transform.py first."
        )
...
def plot_choropleth_static(gdf: gpd.GeoDataFrame, output_dir: Path) -> None:
    ...
    if "composite_score" not in gdf.columns or gdf["composite_score"].isna().all():
        ...
        return
    has_geom = hasattr(gdf, "geometry") and gdf.geometry is not None and gdf.geometry.notna().any()
    if not has_geom:
        logger.warning("No geometry — skipping choropleth map")
        return
    ...
    plt.close()
    logger.success(f"Static choropleth saved: {out}")
...
def plot_choropleth_interactive(gdf: gpd.GeoDataFrame, output_dir: Path) -> None:
    ...
    logger.success(f"Interactive choropleth saved: {out}")
...
def plot_cluster_map(gdf: gpd.GeoDataFrame, output_dir: Path) -> None:
    ...
    plt.savefig(out, dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor())
    plt.close()
    logger.success(f"Cluster map saved: {out}")
...
def plot_inequality_heatmap(output_dir: Path) -> None:
    ...
    plt.close()
    logger.success(f"Inequality heatmap saved: {out}")
...
def plot_scatter_dev_population(gdf: gpd.GeoDataFrame, output_dir: Path) -> None:
    ...
    fig.write_html(str(out))
    logger.success(f"Scatter plot saved: {out}")
...
def main(output_dir: str, fmt: str):
    ...
    logger.success(f"All visualisations written to {out}")
